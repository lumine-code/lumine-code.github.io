...
Install, Update, and Replace are **transactional**: everything is prepared in staging first; the active copy is unloaded and backed up; the swap is atomic; and any failure rolls back to the backup and reloads the previous copy. Installs and uninstalls run asynchronously so the editor stays responsive.
...
- **Fetch** (the refresh action) re-reads the catalogs. The visible list and any previous error notice are **cleared first**, then cards are rebuilt incrementally as records hydrate, so you watch the index fill in rather than waiting for the whole set. A repository that fails to hydrate is recorded as an _error_ with a reason. The panel shows `processed / total / errors`, the time of the last Fetch, and a **Cancel** button; **hovering the progress counter lists the repositories that failed and why**. Cancelling stops new work and leaves the cache consistent.
