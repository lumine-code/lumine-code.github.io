...
Without a selection, `jupyter-repl:run` detects what to execute from the cursor position — a Python compound statement, a multi-line bracket expression, a foldable block, or the current line — and `jupyter-repl:run-and-move-down` advances to the next block afterward. `jupyter-repl:run-all-inline` and its `-above`/`-below` variants run a whole file one statement at a time, `jupyter-repl:recalculate-all-inline` restarts the kernel and re-runs them, and `jupyter-repl:clear-results` removes inline output. Running whole `# %%` cells is the [jupyter-cells](#jupyter-cells) package's job.
...
jupyter-repl is the execution engine behind the [jupyter-view](jupyter-view.md) notebook editor, and behind jupyter-cells' notebook import and export. `jupyter-repl:open-terminal` attaches a Jupyter console to the active kernel in an embedded [terminal](terminal.md) pane, sharing state with your inline code; `jupyter-repl:spawn-terminal` runs it in the system terminal, and `jupyter-repl:copy-console-command` copies the command.
