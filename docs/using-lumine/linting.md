...
A provider is offered every editor there is, which is more than the documents on screen: a package builds editors of its own to render a diff with, or a patch preview, or a commit box in a dock, or the field inside a picker. A provider keyed on a file path never notices, because those have none — but one that is not can say so, and then only the documents open in the centre reach it. A buffer nobody has saved yet still counts.
