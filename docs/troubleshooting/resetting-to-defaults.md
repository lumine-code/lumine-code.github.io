...
Layout, open files, and other per-window state are remembered between sessions. If a window reopens broken, clear that state:

```sh
lumine --clear-window-state
```

This resets the remembered environment without touching your settings, keymap, or packages.
